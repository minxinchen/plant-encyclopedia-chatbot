#!/usr/bin/env python3
"""Continuously refresh validated non-canonical chunks as upstream lanes grow."""
...
def fingerprint(paths: list[Path]) -> str:
    ...
    for path in paths:
        ...
        if path.is_file():
            ...
            for child in sorted(path.glob("*.naming.json")):
...
def main() -> None:
    ...
    sources = [
        args.root / "integration/embedding-ready-candidate-manifest.json",
        args.root / "naming/staging",
    ]
    status_path = args.root / "chunks-candidate/watch-status.json"
    ...
    while True:
        ...
        if current != state["last_fingerprint"]:
            build_code, build_output = run([
                "python3", str(LAB / "scripts/build-preembedding-chunks.py"), "--root", str(args.root)
            ])
            ...
            # A naming lag is expected while the structure maker is growing.
            state["status"] = "caught_up" if not build_code and not validate_code else "awaiting_upstream"
            write_status(status_path, state)

        integration = json.loads(sources[0].read_text(encoding="utf-8"))
